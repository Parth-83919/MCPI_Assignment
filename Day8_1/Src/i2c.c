...
void I2CStop(){
	I2C1->CR1 |= I2C_CR1_STOP;//stop bit after one byte is transfered
...
void I2CSendSlaveAddress(uint8_t slaveaddr){
	...
	(void)I2C1->SR1;
	(void)I2C1->SR2;//claer addr condition
}
...
uint8_t I2CRecvDataNAck(void){
	uint8_t val;
	I2C1->CR1 &= ~(I2C_CR1_ACK | I2C_CR1_POS);
	// wait until recevie buffer is not empty
	while(!(I2C1->SR1 & I2C_SR1_RXNE));
	//read content and clear flags
	val = I2C1->DR;
	return val;
...
/*
	RCC->AHB1ENR |= RCC_AHB1ENR_GPIOBEN;
	GPIOB->MODER &= ~(BV(I2C1_SCL*2));
	GPIOB->MODER |= BV(I2C1_SCL*2+1);
	GPIOB->OTYPER |= BV(I2C1_SCL);
	GPIOB->PUPDR &= ~(BV(I2C1_SCL*2+1) | BV(I2C1_SCL*2));
//	GPIOB->OSPEEDR &= ~(BV(I2C1_SCL*2+1) | BV(I2C1_SCL*2));
	GPIOB->MODER &= ~(BV(I2C1_SDA*2));
	GPIOB->MODER |= BV(I2C1_SDA*2+1);
	GPIOB->OTYPER |= BV(I2C1_SDA);
	GPIOB->PUPDR &= ~(BV(I2C1_SDA*2+1) | BV(I2C1_SDA*2));
*/
	//choose AF4 for I2C1 in alternate function regr
	...
	// fPCLK1 must be at least 2 Mhz for SM mode (100 kHz)
	    ...
	    // CCR = 80
	I2C1->CR2 |= (16 << I2C_CR2_FREQ_Pos);//16 Mhz PCLK
	...
	I2C1->TRISE |= (17 << I2C_TRISE_TRISE_Pos);//program Trise to 17
	//enable I2C
	I2C1->CR1 |= I2C_CR1_PE;
}
